...
ssh $SSH_USER@$SSH_HOST <<EOF
    ...
    # Stop all running services defined in the Docker Compose files, skipping those with 'prod' in the name
    for yml_file in *.yml; do
        if [[ -f "\$yml_file" && "\$yml_file" != *prod*.yml ]]; then
            echo "Stopping services defined in \$yml_file..."
            docker-compose -f "\$yml_file" down
        elif [[ "\$yml_file" == *prod*.yml ]]; then
            echo "Skipping production file: \$yml_file"
        else
