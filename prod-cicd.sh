...
DOCKER_COMPOSE_FILE="prod-docker-compose.yml"  # Name of your Docker Compose file
...
ssh "${DROPLET_USER}@${DROPLET_IP}" "export MAIN_DOMAIN=rutvik2611.com && export url= && export pass= && docker-compose -f ${REMOTE_DIR}/${DOCKER_COMPOSE_FILE} up -d"

# Check if docker-compose up command succeeded
if [ $? -ne 0 ]; then
    echo "Bringing up prod-docker-compose.yml failed." >&2
    exit 1
fi

echo "prod-docker-compose.yml deployed successfully."
